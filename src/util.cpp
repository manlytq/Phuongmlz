...
const char * const BITCOIN_CONF_FILENAME = "xeon.conf";
const char * const BITCOIN_PID_FILENAME = "xeond.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "xeon" is a composite category enabling all Xeon-related debug output
            if(ptrCategory->count(string("xeon"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\XeonCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\XeonCore
    // Mac: ~/Library/Application Support/XeonCore
    // Unix: ~/.xeoncore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "XeonCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/XeonCore";
#else
    // Unix
    return pathRet / ".xeoncore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty xeon.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override xeon.conf
